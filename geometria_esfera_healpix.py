...
data = pd.read_csv('dados.txt',header=0, sep = '\s+', dtype ='a')
...
pixel = np.loadtxt("Pixels_Carlos.txt")
...
for i in range(0,3072):
    ...
    x = r*np.cos(pixel[i,1])*np.sin(pixel[i,2])
    y = r*np.sin(pixel[i,1])*np.sin(pixel[i,2])
    z = r*np.cos(pixel[i,2])
    ...
    if i == 0:
        ...
        final = final.append(aux, ignore_index = True)

#fim do for

np.savetxt('test_file_beta.txt', final,comments ='', fmt='%f', delimiter="\t", header="D	\tz	\tSN	\tID	\tE\t	X\t	Y\t	Z	\tPx\t	Py\t	Pz\t	SN0\t	ID0\t	E0\t	X0\t	Y0\t	Z0\t	P0x\t	P0y\t	P0z\t	SN1\t	ID1\t	E1\t	X1\t	Y1\t	Z1\t	P1x\t	P1y\t	P1z\t	W\t Ang_linha_de_visada  \t Dist_ang")
